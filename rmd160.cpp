...
void Sum(struct digest *d,uint8_t digest[20]) {
  ...
  for( int i = 0 ; i < 5 ;i++){
    uint32_t s = d->s[i];
    digest[i*4] = (uint8_t)(s);
    digest[i*4+1] = (uint8_t)(s>>8);
    digest[i*4+2] = (uint8_t)(s>>16);
    digest[i*4+3] = (uint8_t)(s>>24);
  }
