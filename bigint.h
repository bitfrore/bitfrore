...
struct bigint{
  ...
  public:
    ...
    bigint(uint8_t neg,WORD *data,int cap);
    ...
    void QuoRem(const struct bigint &x,const struct bigint &y,struct bigint &r,WORD vbuf[BIGNATBUFSIZE(32)+1]);
    ...
    void SetBytes(uint8_t neg,const uint8_t *data,int len);

    const struct bignat &AbsValue() const;
    int Sign() const;
