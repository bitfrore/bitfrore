...
OCXX_SRCS = $(wildcard $(patsubst %, %/*.$(OCXX_EXT), . $(SRCS_DIRS)))
...
ifeq ($(REMOVE_UNUSED), 1)
	LD_FLAGS += -s -Wl,--gc-sections -nostartfiles
	OPTIMIZATION += -s -ffunction-sections -fdata-sections
	O3OPTIMIZATION += -s -ffunction-sections -fdata-sections
endif
...
OCXX_OBJS = $(addprefix $(OUT_DIR_F), $(notdir $(OCXX_SRCS:.$(OCXX_EXT)=.o)))
C_OBJS = $(addprefix $(OUT_DIR_F), $(notdir $(C_SRCS:.$(C_EXT)=.o)))
AS_OBJS = $(addprefix $(OUT_DIR_F), $(notdir $(AS_SRCS:.$(AS_EXT)=.o)))
OBJS = $(AS_OBJS) $(C_OBJS) $(CXX_OBJS) $(OCXX_OBJS) $(USER_OBJS)
DEPS = $(OBJS:.o=.d)
...
OCXX_FLAGS_F = $(CORE_FLAGS) $(OOPTIMIZATION) $(CXX_WARNINGS) $(CXX_FLAGS)  $(CXX_DEFS) -MD -MP -MF $(OUT_DIR_F)$(@F:.o=.d) $(INC_DIRS_F)
...
#-----------------------------------------------------------------------------#
# compiling - OC++ source -> objects
#-----------------------------------------------------------------------------#

$(OUT_DIR_F)%.o : %.$(OCXX_EXT)
	@echo 'Compiling file: $<'
	$(CXX) -c $(OCXX_FLAGS_F) $< -o $@
	@echo ' '
